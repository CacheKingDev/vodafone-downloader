...
> **Unofficial project.** This application is not authorized, endorsed, or reviewed by Vodafone. It automates login against the public customer portal and reuses the same client credentials the official web frontend itself ships with (see `src/infrastructure/vodafone/api-client.ts`). Vodafone may change or revoke this access at any time; use is at your own risk and it is your responsibility to comply with your own Vodafone contract's terms of service.
...
- **Web UI** (login, dashboard, accounts, invoices, settings, runs, logs) built on Fastify + htmx
